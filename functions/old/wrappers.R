...
boilerplate_measures_text <- function(
  ...
  methods_db <- check_and_create_templates(db)
...
check_and_create_templates <- function(existing_db = NULL) {
  ...
  for (template_name in names(required_templates)) {
    # Try to add template (will fail if it already exists)
    tryCatch({
      db <- boilerplate_manage_text(
        category = "methods",
        action = "add",
      ...
      modified <- TRUE
      message(paste("Added template:", template_name))
    }, error = function(e) {
  ...
  # Always return the database to ensure it contains the templates
  return(db)
}
