...
#'   action = "save",
#'   db = measures_db,
#'   file_path = "path/to/measures_db.rds"
#' )
#'
#' @export
  ...
  if (!is.null(name) && grepl("\\.", name)) {
    ...
    if (action == "list") {
      ...
      if (is.null(file_path)) {
        file_path <- get_default_measures_file_path()
      }
      return(save_measures_db(db, file_path))
    }
  }
...
#'
#' @noRd
save_measures_db <- function(db, file_path) {
...
#' Get Default File Path for Measures Database
#'
#' @return Character. The default file path for measures database.
#'
#' @noRd
get_default_measures_file_path <- function() {
  # use appropriate package function for path resolution if available
  if (requireNamespace("here", quietly = TRUE)) {
    return(here::here("boilerplate", "data", "measures_db.rds"))
  } else {
    # fallback
    data_dir <- system.file("boilerplate", "data", package = "boilerplate", mustWork = FALSE)
    if (dir.exists(data_dir)) {
      return(file.path(data_dir, "measures_db.rds"))
    } else {
      return(file.path("boilerplate", "data", "measures_db.rds"))
    }
  }
}
