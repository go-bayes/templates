...
# testing
causal_contrast_engine <- function(df, Y, X, baseline_vars = NULL, treat_0 = 0, treat_1 = 1,
                                   estimand = c("ATE", "ATT"), type = c("RR", "RD"), nsims = 200,
                                   cores = parallel::detectCores(), family = "gaussian", weights = TRUE,
                                   continuous_X = FALSE, splines = FALSE, vcov = vcov, verbose = FALSE) {
  # Check if required packages are installed
  ...
  # remainder of the function is as before...
}
...
# the estimator
double_robust <- function(df, Y, X, new_name, baseline_vars = NULL, treat_0 = 0, treat_1 = 1,
                          estimand = "ATE", scale = c("RR","RD"), nsims = 200, cores = parallel::detectCores(),
                          family = "gaussian", weights = TRUE, continuous_X = FALSE, splines = FALSE,
                          delta = 1, sd = 1, type = c("RD", "RR"), vcov = "HC") {

  # Call the causal_contrast_engine() function
  causal_contrast_result <- causal_contrast_engine(df, Y, X, baseline_vars, treat_0, treat_1,
                                                   estimand, scale, nsims, cores, family, weights,
                                                   continuous_X, splines, vcov = "HC")
