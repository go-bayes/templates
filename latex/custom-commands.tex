...
\node [draw=none, inner sep = 1]  (R) at (1, 0) {$R$};
...
\node [rectangle,  draw=red, inner sep = 2] (L1) at (1.5, .5) {$L_{UY}$};
...
\begin{tikzpicture}
\node [rectangle, draw=white, inner sep = 2, thick] (U1) at (0,.25) {U1};
...
\node [draw = none, inner sep = 1] (R)  at (2, 0) {$R$};
...
\begin{tikzpicture}
\node [draw = none, inner sep = 1] (U1) at (0,.25) {U1};
\node [draw = none, inner sep = 1] (U2) at (0,-.25) {U2};
\node [rectangle, draw=black, align=left, inner sep =1] (S) at (1, 0) {\footnotesize $S = 1$};
\node [rectangle, draw=black, inner sep = 2] (A) at (2, 0) {$A_{1}$};
\node [draw = none, inner sep = 1] (Y) at (3, 0) {$Y_{2}$};
\node [rectangle, draw=black, align=left, inner sep =1] (L) at (1,.5) {$L_{0}$};
...
\node [draw = none, inner sep = 1] (R)  at (1.75, 0) {$R$};
...
\node [rectangle, draw=black,align=left] (L) at (1, 0) {\footnotesize $L_{0}$\\
\footnotesize $A_{0}$\\ \footnotesize $Y_{0}$};
\node [rectangle] (A) at (2.5, 0) {$A_1$};
\node [rectangle, draw=white,text=black] (Y) at (3.5,0) {$Y_2$};
\draw [-latex] (U) to (L);
...
\node [circle, inner sep = 1, dashed, draw=blue, thick] (U) at (6.25, 0) {$U_{\Delta Z^T}$}; %
\node [rectangle,  draw=red, thin](YS) at (9, 0) {$Y^{T=1}_{2}$};
...
$\bar{A}$ & Multiple sequential treatments: $A_1, A_2, A_3 \dots$ \\\bottomrule
\end{tabular}
}
...
\textbf{Collider} &  $\immoralityChild$   &  $X_2$ is affected by both $X_0$ and $X_1$, which are independent; conditioning on $X_2$ induces association between $X_0$ and $X_1$. & If \boxed{X_2} or \boxed{X_3} (a descendant of $X_2$), then $X_0 \cancel\coprod X_1 | X_2$. Equally, because $X_3$ is associated with $X_2$, then $X_0 \cancel\coprod X_1 | X_3$ \\
...
$\rightarrowNEW$ & \textbf{Path with an arrow} causal association & $A_1 \rightarrowNEW Y_2$ \\
...
\bigskip
\begin{tabular}{>{\raggedright\arraybackslash\small}m{6cm} >{\raggedright\arraybackslash\small}m{4cm} >{\raggedright\arraybackslash\small}m{4cm}  }
\toprule
\textbf{\small Bias} & \textbf{\small Problem} & \textbf{\small Sequential data solution} \\ \midrule
\textbf{Common cause}: a shared cause creating an association between treatment and outcome, absent direct causation & $\commoncauseA$  &  \commoncausesolvedA\\
\textbf{Mediator bias}: conditioning on a mediator may bias the assessment of the total causal effect between treatment and outcome & $\mediatorA$   &  \commoncausesolvedA \\
\textbf{Collider}: conditioning on a common effect of treatment and outcome induces a spurious association, absent direct causation & $\colliderALONG$  &  \commoncausesolvedA \\
\textbf{Collider by descent}: conditioning on a descendant of a collider induces an association between treatment and outcome absent causation & $\descendantBBLONG$  &  \commoncausesolvedA \\
\bottomrule
% \multicolumn{4}{p{15cm}}{\footnotesize Note 1: In confounding analysis, the biased path (e.g., $A \rightarrowdottedred Y$) is not typically drawn when assessing causal independence between treatment and outcome. However, to assess bias in the true causal effect, the path of bias may be explicitly shown.} \\
\multicolumn{3}{p{15cm}}{\footnotesize Here, our interest is in estimating the total effect of $A \rightarrowNEW Y$. Notice that the concept of `confounding' is relative to a specific causal interest. For example, in causal mediation analysis, conditioning on a mediator is deliberate because we hope to decompose a total effect into its direct and indirect components.} \\
\end{tabular}
}
\newcommand{\terminologyelconfoundersLONG}{
\renewcommand{\arraystretch}{1.5} % line spacing
\centering
% \begin{tabular}{>{\raggedright\arraybackslash\small}m{6cm} >{\raggedright\arraybackslash\small}m{3.5cm} >{\raggedright\arraybackslash\small}m{3.5cm}}\\
% \multicolumn{3}{c}{\textbf{Elementary Confounding}}\\
% \toprule
...
% \textbf{M-bias}: Over-conditioning bias. Response: do not condition on $L$ & $\mbias$ & $\mbiassolved$ \\
% \textbf{Unmeasured common cause} If a proxy of an unmeasured confounder is not a mediator or collider, conditioning will reduce bias even if the proxy occurs after the outcome.& $\downstream$ & $\downstreamconfounder$ \\ 
% \textbf{Unmeasured common cause} Conditioning on baseline exposure and baseline outcome exerts powerful confounding control and recovers incident effect. & \downstream & \three \\
% \bottomrule
% \end{tabular}
\centering
\begin{tabular}{>{\raggedright\arraybackslash\small}m{0.5cm} >{\raggedright\arraybackslash\small}m{5.5cm} >{\raggedright\arraybackslash\small}m{3.5cm} >{\raggedright\arraybackslash\small}m{3.5cm}}\\
\multicolumn{4}{c}{\textbf{Elementary Confounding}}\\
\toprule
& \multicolumn{3}{c}{\bf \small Simple scenarios}\\
& \textbf{\small Bias} & \textbf{\small Problem} & \textbf{\small Response} \\
\midrule
1 & \textbf{Common cause}: a shared cause creating an association between treatment and outcome, absent direct causation & $\commoncauseA$  &  \commoncausesolvedA\\
...
7 & \textbf{Unmeasured common cause} Conditioning on baseline exposure and baseline outcome exerts powerful confounding control and recovers incident effect. & \downstream & \three \\
\bottomrule
\end{tabular}
...
\toprule
\textbf{\small Bias} & \textbf{\small Problem} & \textbf{\small Sequential data solution} \\ \midrule
\textbf{Reverse causation}: incorrectly assuming $Y$ is caused by $A$. & $\ytoa$ & $\aandysolution$ \\
\textbf{Incorrect timing of exposure/outcome}: $A$ is mistakenly timed after $Y$. & $\atwotoyone$ & $\aandysolution$ \\
\textbf{Measured common cause}: the typical problem we hope to solve: an unmeasured variable is assumed to cause both $A$ and $Y$. & $\commoncauseA$ & $\commoncausesolvedA$ \\
\textbf{Unmeasured common cause} conditioning on baseline exposure and baseline outcome exerts powerful confounding control and recovers incident effect. & \downstream & \three \\
\textbf{Mediator bias}: conditioning on a mediator, which can distort the true causal effect. & $\mediatorA$ & $\commoncausesolvedA$ \\
\textbf{Collider bias}: controlling for a variable that is affected by both $A$ and $Y$ can introduce spurious association. & $\colliderALONG$ & $\commoncausesolvedA$ \\
\textbf{Collider proxy bias}: conditioning on a descendant of a collider can introduce bias. & $\descendantBBLONG$ & $\commoncausesolvedchild$ \\
\textbf{Post-exposure collider stratification bias}: conditioning on a variable that is affected by exposure but does not have a direct causal effect on outcome. & $\mediatorcollider$ & $\commoncausesolvedA$ \\
\bottomrule
\multicolumn{3}{p{15cm}}{\footnotesize Correctly aligning confounders, exposures, and outcomes with the arrow of time avoids common scenarios of confounding.} \\
\end{tabular}
}
...
\begin{tabular}{>{\raggedright\arraybackslash\footnotesize}m{5cm} >{\raggedright\arraybackslash\footnotesize}m{5cm} >{\raggedright\arraybackslash\footnotesize}m{4.0cm}}
\toprule
\textbf{Bias} & \textbf{\footnotesize{Problem}} & \textbf{\footnotesize Response} \\ \midrule
\textbf{M-bias}: Over-conditioning bias. Response: do not condition on $L$ & $\mbias$ & $\mbiassolved$ \\
\textbf{M-bias, L is a confounder}: Pre-treatment collider $L$ is also a confounder. \textbf{Response}: adjust for proxies of unmeasured confounders that do not induce M-bias. & $\mbiasdoomed$ & $\mbiasdoomedsolved$ \\
\textbf{Unmeasured confounding} \textbf{Response}: unless a proxy of an unmeasured confounder is a mediator or collider, conditioning this proxy will reduce bias even if the proxy occurs after the outcome.& $\downstream$ & $\downstreamconfounder$ \\
\textbf{Residual confounding} \textbf{Response}: sensitivity analysis. & $\threeshortB$ &  \footnotesize{Sensitivity analysis} \\
\textbf{Treatment affects confounder of mediator/outcome path} \textbf{Response}: special estimators& $\mediationfullshortB$ & {Special estimators}\\
\textbf{Treatment/confounder feedback}: feedback in sequential exposures raises special problems. Here: to avoid confounding, we must condition on $L$; yet doing so leads to collider confounding. \textbf{Response}: special estimators & $\feedbackAshortB$ & \footnotesize{Special estimators} \\
\bottomrule
\end{tabular}
}
...
\renewcommand{\arraystretch}{1.5} % line spacing
\centering
\begin{tabular}{>{\raggedright\arraybackslash\small}m{4cm} >{\raggedright\arraybackslash\small}m{6.5cm} >{\raggedright\arraybackslash\small}m{3.5cm}}\\
\multicolumn{3}{c}{\textbf{Confounding in experiments}}\\
\toprule
% \textbf{\small Bias} & \textbf{\small Problem} & \textbf{\small Response} \\ \midrule
% \textbf{M-bias when selecting into study}:  & $\attritionBshort$  &  \attritionBBshort\\
\textbf{Post-exposure 'control'}:  & $\mediatorA$  &  \commoncauseALATENTshort\\
\textbf{Post-exposure collider stratification bias} & $\mediatorcollider$ & $\commoncauseALATENTshort$ \\
\textbf{Treatment affects censoring} & $\sdCshortVERY$ & Inverse probability of censoring weights \\
% \textbf{Treatment/confounder feedback}: Feedback in sequential exposures raises special problems. Here: to avoid confounding, we must condition on $L$; yet doing so leads to collider confounding & $\feedbackAshortB$ & \footnotesize{Special estimators} \\
\textbf{Per protocol: unmeasured variable affects adherence} & \perprotocolONE & Condition on \boxedred{L_0} and  \boxedred{L_1} \\
\textbf{Per protocol: unmeasured variable and treatment both affect adherence} & \perprotocolTWO  &  Use special estimators\\
\bottomrule
% \multicolumn{4}{p{15cm}}{\footnotesize Note 1: In confounding analysis, the biased path (e.g., $A \rightarrowdottedred Y$) is not typically drawn when assessing causal independence between treatment and outcome. However, to assess bias in the true causal effect, the path of bias may be explicitly shown.} \\
% \multicolumn{3}{p{15cm}}{\footnotesize Here, our interest is in estimating the total effect of $A \rightarrowNEW Y$. Notice that the concept of `confounding' is relative to a specific causal interest. For example, in causal mediation analysis, conditioning on a mediator is deliberate because we hope to decompose a total effect into its direct and indirect components.} \\
\end{tabular}
...
\bigskip
\begin{tabular}{>{\raggedright\arraybackslash\footnotesize\bf}m{0.5cm} >{\raggedright\arraybackslash\footnotesize}m{7cm} m{4.5cm}}
\toprule
\textbf{No.} & \textbf{Type} & \textbf{} \\ \midrule
...
\begin{tabular}{>{\raggedright\arraybackslash\footnotesize}m{4.5cm} >{\raggedright\arraybackslash\footnotesize}m{10cm}} % Adjust column widths for a better fit
\toprule
\textbf{Bias} & \textbf{Causal Diagram} \\ \midrule
\textbf{Uncorrelated errors under sharp null: no treatment effect}: Under sharp null, uncorrelated measurement errors do not lead to bias. & \measurementUNCORshort \\
\textbf{Uncorrelated errors except under sharp null}:  Outside sharp null, uncorrelated measurement errors distort targeted effects.$^{*}$ & \measurementUCORBshort \\
\textbf{Correlated errors}: related, systematic errors in $A$ and $Y$ measurements that are related. & \measurementCORshort \\
\textbf{Directed error: exposure effects error of outcome}: $A$ affects $Y$'s measurement error. & \measurementDIRshort \\
\textbf{Directed error: outcome affects error of exposure}:  $Y$ affects $A$'s measurement error. &  $\measurementYshort$\\
\textbf{Correlated/directed error}: both systematic and correlated errors in $A$ and $Y$ measurements are from unmeasured source of dependency. & \measurementCORDIRshort \\
\bottomrule
%\bottomrule
%\multicolumn{2}{m{15cm}}{\footnotesize$^{*}$In graph 2, uncorrelated errors {$ \Biggl\{$\circledottedblue{Z},~\circledottedblue{U_{\Delta Z}}$ \Biggl\}$} are effect-modifiers for $A_1\rightarrowNEW Y_2$ in the target population.} \\
\end{tabular}
}
...
\begin{tabular}{>{\raggedright\arraybackslash\footnotesize\bf}m{.35cm} >{\raggedright\arraybackslash\footnotesize}m{4.5cm} >
...
4.& No treatment effect and outcome causes censoring: {\it \bf  no bias}  & $\sdDshort$ \\
5.& Treatment effect and outcome causes censoring {\it \bf bias}  & $\sdDDshort$ \\
...
% \centering
\centering
\textbf{External Validity (restriction/eligibility at baseline)}\\
%\medskip
\renewcommand{\arraystretch}{.75} % Adjust the line spacing
\centering
\begin{tabular}{>{\raggedright\arraybackslash\footnotesize\bf}m{0.15cm} >{\raggedright\arraybackslash\footnotesize}m{4cm} >{\raggedright\arraybackslash\footnotesize}m{10cm}}
\toprule
...
% & \textbf{Response:} (a) do not restrict sample; (b) condition on descendent of $U$ & $\attritionBB$ \\
% \cmidrule{1-3}
2 & \textbf{Problem:} sample is too restrictive. Sample ATE does not transport to target population. & $\sdGshortTARGET$ \\
& \textbf{Response:} sample from targeted (unrestricted) population. & $\sdGshortTARGETresponse$ \\
\cmidrule{1-3}
3 & \textbf{Problem:} absence of eligibility criteria. & $\sdGshortTARGETTWO$ \\
& \textbf{Response:} eligibility criteria. & $\sdGshortTARGETTWOresponse$ \\
\cmidrule{1-3}
4 & \textbf{Problem:} unmeasured correlated treatment/outcome errors in a heterogeneous target sample. & $\measurementCORDIRshortCULTURE$ \\
& \textbf{Response:} eligibility criteria. & $\measurementCORDIRshortCULTUREresponse$ \\
\cmidrule{1-3}
5 & \textbf{Problem:} Unmeasured correlated control/treatment errors in a heterogeneous target sample. & $\restrictionbaselineCULTURE$ \\
