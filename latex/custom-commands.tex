...
\draw [-latex, draw = red] (A) to (L);
\draw [-latex, draw = red] (L) to (Y);
\draw [-latex,  bend left=30, draw=red,dashed] (A) to node[pos=0.5, above,draw=none, text = red] {\footnotesize attenuated total effect} (Y);
\end{tikzpicture}
}
\newcommand{\collider}{
\begin{tikzpicture}
...
\begin{tikzpicture}
\node [rectangle, draw=black, inner sep = 2, thick] (U1) at (0,.25) {U1};
\node [draw = none, inner sep = 1] (U2) at (0,-.25) {U2};
\node [rectangle, draw=red, align=left] (S) at (2, 0) {$S = 1$};
\node [draw = none, inner sep = 1] (A) at (3, 0) {$A_{1}$};
\node [draw = none, inner sep = 1] (Y) at (4, 0) {$Y_{2}$};
...
\node [rectangle, draw=white,text=black] (Y) at (4.5,0) {$Y_2$};
\draw [-latex] (U) to (L);
...
$Z$ & Effect-modifief or `moderator' of $A$ on $Y$\\
