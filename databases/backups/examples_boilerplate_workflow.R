...
str(measures_db, max.level = 1)  # Just see top-level entries
str(measures_db, max.level = 2)
# ------------------------------------------------------
# 2. Add a new measure (top-level)
# ------------------------------------------------------
measures_db <- boilerplate_manage_measures2(
  action = "add",
  name = "binge_drinking_frequency",
  measure = list(
    description = "Frequency of binge drinking was measured using a single item.",
    reference = "nzavs2018",
    waves = "10-current",
    keywords = c("alcohol", "binge", "drinking", "frequency"),
...
print(alcohol_measure)

# Get a nested measure
...
# Remove a measure
temp_db <- boilerplate_manage_measures2(
  action = "remove",
  name = "binge_drinking_frequency",
  db = temp_db
)
...
# Verify it's gone
"binge_drinking_frequency" %in% names(temp_db)
...
measures_bibliography <- boilerplate_report_measures(
  all_vars = c(
    "alcohol_frequency",
    "psychological.anxiety.social",
    "psychological.depression.phq9"
  ),
...
study_params <- list(
  ...
  outcome_var = "psychological.depression.phq9",
  n_total = 47000,
